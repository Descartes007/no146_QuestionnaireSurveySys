		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				<el-row :gutter="20" class="slt"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '问卷名称' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.exampaperName" placeholder="问卷名称"
							clearable></el-input>
					</el-form-item>
					...
					<el-form-item :label="contents.inputTitle == 1 ? '问卷状态' : ''">
						<el-select v-model="searchForm.exampaperTypes" placeholder="请选择问卷状态">
							...
							<el-option v-for="(item,index) in exampaperTypesSelectSearch" v-bind:key="index"
					...
					<el-form-item>
						<el-button type="success" @click="search()">{{ contents.searchBtnFont == 1?'查询':'' }}<i
				...
				<el-row class="ad"
					...
					<el-form-item>
						<el-button v-if="isAuth('examrecord','新增')" type="success" icon="el-icon-plus"
							@click="addOrUpdateHandler()">{{ contents.btnAdAllFont == 1?'新增':'' }}</el-button>
						&nbsp;
						<el-button v-if="isAuth('examrecord','删除')" :disabled="dataListSelections.length <= 0"
							type="danger" icon="el-icon-delete" @click="deleteHandler()">
			...
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					v-if="isAuth('examrecord','查看')" :data="dataList" v-loading="dataListLoading"
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="examrecordUuidNumber" header-align="center" label="问卷调查编号">
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign"
						prop="exampaperName" header-align="center" label="问卷名称">
						<template slot-scope="scope">
							{{scope.row.exampaperName}}
						</template>
					</el-table-column>

					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yonghuName"
						header-align="center" label="用户姓名">
						<template slot-scope="scope">
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="insertTime"
						header-align="center" label="问卷调查时间">
						<template slot-scope="scope">
					...
					<el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
						<template slot-scope="scope">
							<el-button v-if="isAuth('examrecord','查看')" type="success" icon="el-icon-tickets"
								size="mini" @click="viewExamDetails(scope.row.examrecordUuidNumber)">
								{{ contents.tableBtnFont == 1?'查看问卷调查详情':'' }}</el-button>

							<el-button v-if="isAuth('examrecord','修改')" type="primary" icon="el-icon-edit" size="mini"
								@click="addOrUpdateHandler(scope.row.id)">{{ contents.tableBtnFont == 1?'修改':'' }}
							</el-button>

							<el-button v-if="isAuth('examrecord','删除')" type="danger" icon="el-icon-delete" size="mini"
		...
		methods: {
				...
				viewExamDetails(uuid) {
					...
					contentSearchStyleChange() {
						this.$nextTick(() => {
							...
							if (this.contents.inputTitle) {
								document.querySelectorAll('.form-content .slt .el-form-item__label').forEach(
								el => {
							...
							setTimeout(() => {
								document.querySelectorAll('.form-content .slt .el-input__prefix').forEach(
									el => {
										el.style.color = this.contents.inputIconColor
										el.style.lineHeight = this.contents.inputHeight
									})
								document.querySelectorAll('.form-content .slt .el-input__suffix').forEach(
									el => {
										el.style.color = this.contents.inputIconColor
										el.style.lineHeight = this.contents.inputHeight
									})
								document.querySelectorAll('.form-content .slt .el-input__icon').forEach(
									el => {
										el.style.lineHeight = this.contents.inputHeight
									})
							}, 10)
					...
					getDataList() {
						...
						if (this.searchForm.createTimeStartSearch != '' && this.searchForm.createTimeStartSearch !=
							undefined) {
							params['createTimeStartSearch'] = this.searchForm.createTimeStartSearch
						}
						if (this.searchForm.createTimeEndSearch != '' && this.searchForm.createTimeEndSearch !=
							undefined) {
