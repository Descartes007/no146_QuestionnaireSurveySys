...
<template>
    <div class="main-content">
        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '题目名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.examquestionName" placeholder="题目名称" clearable></el-input>
                     </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('examquestion','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "examquestion.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					<el-button v-if="isAuth('examquestion','报表')" type="success" icon="el-icon-pie-chart"
						@click="chartDialog(scope.row.id)">查看本题用户选项统计</el-button>
                        </template>
                    </el-table-column>
                </el-table>
                <el-pagination
		...
		<el-dialog title="统计报表" :visible.sync="chartVisiable" width="800">
			<div id="examquestionChart" style="width:100%;height:600px;"></div>
    ...
    //当前表下拉框搜索条件
            ...
            json_fields: {
                //级联表字段
                     ...
                     '题目类型': "examquestionValue",
                     '题目排序': "examquestionSequence",
            },
        ...
        methods: {
			chartDialog(id) {
				...
				this.$nextTick(() => {
					var examquestionChart = this.$echarts.init(document.getElementById("examquestionChart"), 'macarons');
					this.$http({
						url: "/cal/examredetails?examquestionId="+id,
						method: "get",
					}).then(({
						data
					}) => {
						if (data && data.code === 0) {
							...
							for (let i = 0; i < res.length; i++) {
								xAxis.push(res[i].xuanxiang);
								yAxis.push(res[i].total);
								pArray.push({
									value: res[i].total,
									name: res[i].xuanxiang
								})
								var option = {};
								option = {
									title: {
								...
								// 使用刚指定的配置项和数据显示图表。
								examquestionChart.setOption(option);
								//根据窗口的大小变动图表
								window.onresize = function() {
									examquestionChart.resize();
								};
							}
						}
					});
